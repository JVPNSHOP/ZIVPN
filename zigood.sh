...
function fillForm(u,p,e){
  ...
  f.querySelector('input[name="expires"]').value=e;
  f.scrollIntoView({behavior:'smooth', block:'start'});
}
</script>
<style>
/* tighter table rows */
.table-tight td, .table-tight th { padding-top:.25rem; padding-bottom:.25rem; }
.code-chip { font-family: ui-monospace, SFMono-Regular, Menlo, monospace; }
.truncate-soft { max-width: 220px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
.btn-slim { padding:.25rem .5rem; }
</style>
</head>
...
<header class="bg-gradient-to-r from-slate-900 to-slate-800 text-white">
  <div class="max-w-6xl mx-auto px-4 py-6 flex items-center justify-between">
    ...
    <div class="flex items-center gap-2">
      ...
      <!-- Show Running Host/IP -->
      <span class="text-xs bg-emerald-600/20 ring-1 ring-emerald-400/40 px-2 py-1 rounded-full">
        Host: <b>{{ request.host }}</b>
      </span>
      <button onclick="copyText('http://{{ request.host }}/login', this)" class="btn-slim bg-emerald-600 hover:bg-emerald-500 rounded text-xs">Copy URL</button>
    </div>
  </div>
</header>
          ...
          <tr class="border-t">
            <td class="py-1">
              <div class="flex items-center gap-2">
                ...
                <button onclick="copyText('{{r['username']}}', this)" class="btn-slim bg-slate-800 text-white rounded text-xs">Copy</button>
              </div>
            </td>
            <td class="py-1">
              <div class="flex items-center gap-2 flex-wrap">
                <code class="code-chip px-1.5 py-0.5 bg-slate-100 rounded truncate-soft" title="{{r['password']}}">{{r['password']}}</code>
                <button onclick="copyText('{{r['password']}}',this)" class="btn-slim bg-slate-800 text-white rounded text-xs">Copy</button>
                {% if r['days_left'] is not none %}
                  {% if r['days_left'] >= 0 %}
                    <!-- Show just days number (no 'သက်တော်' text) -->
            ...
            <td class="py-1">
              ...
              {% else %}
                <!-- If expired, still show Offline (for renew UX) -->
                <span class="inline-flex items-center gap-1 text-slate-600"><span class="w-2 h-2 rounded-full bg-slate-400"></span>Offline</span>
              {% endif %}
            </td>
            <td class="py-1">
              <div class="flex items-center gap-2">
                <!-- Edit to prefill the form for quick renew/update -->
                <button type="button" onclick="fillForm('{{r['username']}}','{{r['password']}}','{{r['expires']}}')" class="btn-slim bg-amber-500 hover:bg-amber-400 text-white rounded text-xs">Edit</button>
                <form method=post action="/del/{{r['id']}}" onsubmit="return confirm('Delete {{r['username']}} ?')">
                  <button class="btn-slim bg-rose-600 hover:bg-rose-500 text-white rounded text-xs">🗑️</button>
                </form>
              </div>
            </td>
          </tr>
