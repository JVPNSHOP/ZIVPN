...
cat > zi.sh <<'BASH'
#!/bin/bash
# ZIVPN UDP + Tailwind Web Panel (UI tweaks + shared-password protection)
# Script By: JueHtet
# Modifications: copy/edit safety + expire password when used by 2+ clients (admin must update)
...
def logs():
    try:
        # get recent journal entries for the service; lowercased for searching
        out = subprocess.check_output(["journalctl","-u",ZIVPN_SVC,"-n","2000","--since","-1h","-o","cat"])
        return out.decode(errors='ignore').lower()
    except Exception:
        return ""
...
def active_rows():
    ...
    with db() as con:
        for r in con.execute("SELECT * FROM users"):
            ...
            online=False
            try:
                # compare lowercased occurrences (same logic as logs())
...
def enforce_shared_pw_protection():
    """
    If a password appears >=2 times in recent logs, mark all users that have that password as expired
    (set expires to yesterday). Admin must manually update expires to allow reuse.
    """
    l = logs()
    if not l:
        return
    ...
    with db() as con:
        # get distinct passwords that exist in DB
        pw_list = [r[0] for r in con.execute("SELECT DISTINCT password FROM users")]
        for p in pw_list:
            if not p:
                continue
            # count occurrences in logs (lower)
            if l.count(p.lower()) >= 2:
                # expire all users that have this password and whose expires is >= today
                con.execute("UPDATE users SET expires=? WHERE password=? AND DATE(expires)>=DATE('now')", (yesterday, p))
                # optional: flash an admin-visible message next request cycle
                # we cannot flash from here (not in request context), but the sync() triggered from web will flash
    return

def sync():
    # run shared-password enforcement first
    enforce_shared_pw_protection()
...
@login_required
def index():
    # enforce protection on each web dashboard load too
    enforce_shared_pw_protection()

    rows=active_rows()
    total_users=len(rows)
    total_online=sum(1 for r in rows if not r["expired"] and r["online"])
...
function copyText(t, btn){
  ...
  try {
    if (navigator.clipboard && window.isSecureContext){ navigator.clipboard.writeText(t).then(ok).catch(()=>{ prompt('Copy password', t); ok(); }); }
    else { prompt('Copy password', t); ok(); }
  } catch(e){ prompt('Copy password', t); ok(); }
}
          ...
          <tr class="border-t">
            ...
            <td class="py-1">
              <div class="flex items-center gap-1.5 flex-wrap">
                ...
                <button onclick="copyText({{ r['password']|tojson }},this)" class="btn-slim bg-slate-800 text-white rounded text-[11px]">Copy</button>
            ...
            <td class="py-1">
              ...
              {% elif not r['expired'] and not r['online'] %}
                <span class="inline-flex items-center gap-1 text-slate-600"><span class="w-2 h-2 rounded-full bg-amber-400"></span>Idle</span>
              {% else %}
            ...
            <td class="py-1">
              <div class="flex items-center gap-1.5">
                <button type="button" onclick="fillForm({{ r['username']|tojson }}, {{ r['password']|tojson }}, {{ r['expires']|tojson }})" class="btn-slim bg-amber-500 hover:bg-amber-400 text-white rounded text-[11px]">Edit</button>
...
def enforce_shared_pw():
    """
    Check recent journal logs for duplicate password usage and expire those users (set to yesterday).
    """
    try:
        logs = subprocess.check_output(["journalctl","-u",SVC,"-n","2000","--since","-1h","-o","cat"]).decode().lower()
    except Exception:
        logs = ""
    if not logs:
        return
    ...
    with sqlite3.connect(DB) as con:
        cur = con.cursor()
        cur.execute("SELECT DISTINCT password FROM users")
        for row in cur.fetchall():
            p = row[0]
            if not p: continue
            if logs.count(p.lower()) >= 2:
                cur.execute("UPDATE users SET expires=? WHERE password=? AND DATE(expires)>=DATE('now')", (yesterday, p))
        con.commit()

# run enforcement first
enforce_shared_pw()
