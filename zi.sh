...
<script>
// Robust copy + date picker helper + dismiss notice
...
<!-- (moved) Logout: small button on top-right -->
  ...
  <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
    ...
    <div class="flex items-center gap-2">
      ...
      <span class="text-[11px] bg-emerald-600/20 ring-1 ring-emerald-400/40 px-2 py-1 rounded-full">
        Host: <b>{{ request.host }}</b> • VPS IP: <b>{{ vps_ip }}</b>
      </span>
...
def save():
    ...
    # Prepare success message with VPS IP + details
    try:
