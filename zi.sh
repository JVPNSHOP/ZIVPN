cat > zi.sh <<'BASH'
#!/bin/bash
# ZIVPN + Web Admin (fixed) — one-shot installer
set -euo pipefail
...
apt-get install -y curl jq python3-venv python3-pip >/dev/null
...
read -rp "Enter passwords (comma separated) [default: zi]: " INPUT_PW
INPUT_PW=${INPUT_PW:-zi}

# Normalize config: fix legacy 'authh' -> 'auth' and set password list
python3 - "$ZIVPN_CFG" "$INPUT_PW" <<'PY'
import sys, json, os
cfg_path, csv = sys.argv[1], sys.argv[2]
pw = [x.strip() for x in csv.split(",") if x.strip()] or ["zi"]

def ensure(cfg):
    # migrate legacy "authh" -> "auth"
    if "authh" in cfg and "auth" not in cfg:
        cfg["auth"] = cfg.pop("authh")
...
echo "==> Install Web Admin Panel (Flask + Waitress)"
mkdir -p "${ADMIN_DIR}"
python3 -m venv "${VENV}"
...
# Flask app (UI + logic) — writes only to auth.config with non-expired users
cat > "${APP_PY}" <<'PY'
#!/usr/bin/env python3
...
def sync_config_and_restart():
    cfg = load_zivpn_config()
    pw = active_passwords() or ["zi"]
    cfg["auth"]["config"] = pw
    ...
    try:
        ...
        subprocess.run(["tee",ZIVPN_CONFIG_PATH], input=text.encode(), stdout=subprocess.DEVNULL, check=False)
...
<style>
:root{--bg:#f6f7fb;--card:#fff;--pri:#14a44d;--danger:#dc3545;--mut:#6b7280}
body{font-family:system-ui,-apple-system,Segoe UI,Roboto,Arial;background:var(--bg)}
.container{max-width:1100px;margin:2rem auto;padding:0 1rem}
.card{background:var(--card);border-radius:14px;box-shadow:0 6px 18px rgba(0,0,0,.06);padding:1rem}
.grid{display:grid;grid-template-columns:320px 1fr;gap:1rem}
input,select{width:100%;padding:.6rem;border:1px solid #e3e3ea;border-radius:10px;margin:.25rem 0 1rem}
.btn{padding:.6rem .9rem;border:0;border-radius:10px;background:var(--pri);color:#fff;font-weight:700;cursor:pointer}
...
header{display:flex;gap:.5rem;justify-content:space-between;align-items:center;margin-bottom:1rem}
small.mut{color:var(--mut)}
.badge{padding:.3rem .5rem;border-radius:6px;background:#eef2ff;color:#3730a3;font-weight:600}
</style>
</head><body><div class="container">
<header>
  <div>
    <h2>🛡️ {{title}}</h2>
    <small class="mut">Server: <span class="badge">{{srv_ip}}</span> • Panel: http://{{srv_ip}}:{{port}}/login</small>
  </div>
  <div>
    <form style="display:inline" method="post" action="{{url_for('sync')}}"><button class="btn" type="submit">Save + Sync</button></form>
    <a class="btn" style="background:#0ea5e9" href="/logout">Logout</a>
  </div>
</header>
...
@app.route("/login")
def login():
    return Response("Use HTTP Basic Auth (username/password) in this browser session.", 401,
        {"WWW-Authenticate":'Basic realm="ZIVPN Admin"'})

@app.route("/logout")
def logout():
    return Response("Logged out", 401, {"WWW-Authenticate":'Basic realm="ZIVPN Admin"'})

@app.route("/", methods=["GET"])
@require_auth
def index():
    default_expiry = date(date.today().year+1, date.today().month, date.today().day).isoformat()
    return render_template_string(HTML, title=APP_TITLE, rows=list_users(),
        default_expiry=default_expiry, srv_ip=server_ip(), port=BIND_PORT)
...
def sync():
    try:
        sync_config_and_restart(); flash("Synced to auth.config + restarted ZIVPN")
    except Exception as ex:
        flash(f"Sync failed: {ex}")
    return redirect(url_for("index"))

# Serve via Waitress in systemd
app.wsgi_app = app.wsgi_app
PY

chmod +x "${APP_PY}"

# systemd for panel (runs Waitress)
...
echo "Open Panel    : http://${IP}:${PANEL_PORT}/login"
echo "======================================"
BASH

chmod +x zi.sh
echo "Saved ./zi.sh (run: sudo ./zi.sh)"
