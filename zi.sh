cat > zi.sh <<'BASH'
#!/bin/bash
# ZIVPN UDP + Tailwind Web Panel (No auto-restart on save; UI restored)
# Script By: JueHtet
# Rev: Safe Save (no reload), Manual "Apply Config" button
...
# systemd unit (reload supported if daemon handles HUP; but we won't auto-call it on save)
...
def sync():
    # Save-time: only write config (NO reload/restart) to avoid dropping existing users
    with db() as con:
...
{{ msg }}
  ...
  <section class="flex gap-2">
    <form method="post" action="/apply" onsubmit="return confirm('Apply config to server now? (existing users might briefly reconnect if daemon restarts)')">
      <button class="bg-slate-800 hover:bg-slate-700 text-white rounded-xl px-3 py-2 text-sm">⚙️ Apply Config</button>
    </form>
  </section>
      ...
      <table class="w-full text-left align-middle table-tight">
        <thead>
          <tr class="text-slate-600 text-[12px]">
            <th class="py-1">User</th><th class="py-1">Password</th><th class="py-1">Expires</th><th class="py-1">Status</th><th class="py-1"></th>
          </tr>
        </thead>
        <tbody class="tiny">
          {% for r in rows %}
...
@login_required
def apply():
    # Manual apply: try reload first; if reload fails, allow safe restart only with no active UDP
    try:
        subprocess.check_call(["systemctl","reload",ZIVPN_SVC], stdout=DEVNULL, stderr=DEVNULL)
        flash("Applied config with reload ✅ (no disconnect).", "ok")
    except Exception:
        # Fallback: only restart if no active sessions on port 5667
        try:
            c=int(subprocess.check_output("ss -Huan 'sport = :5667' | wc -l", shell=True).decode().strip() or 0)
            if c==0:
                subprocess.Popen(["systemctl","restart",ZIVPN_SVC], stdout=DEVNULL, stderr=DEVNULL)
                flash("No active users detected — restarted service to apply config ✅", "ok")
            else:
                flash(f"Reload not supported. {c} active session(s) — skipped restart to avoid disconnect ❌", "err")
        except Exception:
            flash("Reload failed and could not check sessions. Skipped restart.", "err")
    return redirect("/")
...
# --- Auto Sync Script (never restarts automatically) ---
cat > "${SYNC_PY}" <<'PY'
