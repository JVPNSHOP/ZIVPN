cat > zi.sh <<'BASH'
#!/bin/bash
# ZIVPN UDP + Tailwind Web Panel (1-device-per-password guard)
# Script By: JueHtet
...
# ---- Flask app (previous UI with your latest tweaks kept) ----
cat > "${APP_PY}" <<'PY'
#!/usr/bin/env python3
...
function copyText(t, btn){ if(navigator.clipboard&&window.isSecureContext){navigator.clipboard.writeText(t);} }
  ...
  <section class="grid md:grid-cols-[320px_1fr] gap-3">
    ...
    <div class="bg-white p-3 rounded-2xl shadow ring-1 ring-slate-200 overflow-x-auto">
      <table class="w-full text-left align-middle table-tight">
        ...
        rows=active_rows(), total_users=total_users, total_online=total_online, total_offline=total_offline,
...
# --- 1-device-per-password Guard (new) ---
cat > "${GUARD_PY}" <<'PY'
import os, re, sqlite3, secrets, string, subprocess, time, tempfile, json
DB="/var/lib/zivpn-admin/zivpn.db"
CFG="/etc/zivpn/config.json"
SVC="zivpn.service"
...
def resync_password_list():
    # reload config with current active passwords
    with sqlite3.connect(DB) as con:
        pw=[r[0] for r in con.execute("SELECT DISTINCT password FROM users WHERE DATE(expires)>=DATE('now')")]
    cfg={}
    ...
    while True:
        ...
        with sqlite3.connect(DB) as con:
            ...
            for uid,uname,pw in users():
                ...
                if len(ips) > 1:
                    # rotate password to kick concurrent devices
                    newp=rand_pass(10)
                    con.execute("UPDATE users SET password=? WHERE id=?", (newp, uid))
                    changed=True
        if changed:
...
# systemd units for guard daemon
cat >/etc/systemd/system/${GUARD_SVC} <<EOF
[Unit]
Description=ZIVPN 1-device guard (auto-rotate on multi-IP)
