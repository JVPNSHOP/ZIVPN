cat > zi.sh <<'BASH'
#!/bin/bash
# ZIVPN UDP + Tailwind Web Panel (Real-time Status)
# Script By: JueHtet
...
# Create service with reload capability and better logging
...
def get_zivpn_logs():
    """Get ZIVPN server logs for connection tracking"""
    try:
        # Get logs from last 10 minutes
        logs = subprocess.check_output([
            "journalctl", "-u", ZIVPN_SVC, 
...
def parse_connection_events():
    """Parse ZIVPN logs to detect connection and disconnection events"""
    logs = get_zivpn_logs()
    connection_events = {}
    
    # Patterns to match connection events (adjust based on actual ZIVPN log format)
    connect_patterns = [
        r'connection.*established.*password[=\s:]*([^\s,\]]+)',
        r'client.*connected.*password[=\s:]*([^\s,\]]+)',
        r'auth.*success.*password[=\s:]*([^\s,\]]+)',
        r'password[=\s:]*([^\s,\]]+).*connected',
        r'password[=\s:]*([^\s,\]]+).*established'
    ]
    
    disconnect_patterns = [
        r'connection.*closed.*password[=\s:]*([^\s,\]]+)',
        r'client.*disconnected.*password[=\s:]*([^\s,\]]+)',
        r'password[=\s:]*([^\s,\]]+).*disconnected',
        r'password[=\s:]*([^\s,\]]+).*closed'
    ]
    ...
    for pattern in connect_patterns:
        ...
        for password in matches:
            if len(password) > 2:  # Filter out short/noise matches
                connection_events[password] = True
    ...
    for pattern in disconnect_patterns:
        ...
        for password in matches:
            if len(password) > 2:
                # Only mark as disconnected if we don't see recent connection
                if password in connection_events:
                    # Check if this disconnect is more recent than connect
                    connection_events[password] = False
    
    return connection_events

def get_active_connections():
    """Get currently active connections from logs"""
    return parse_connection_events()

def logs():
    """Get recent logs for password detection"""
    try:
...
def active_rows():
    ...
    # Get active connections from ZIVPN logs
    ...
    with db() as con:
        for r in con.execute("SELECT * FROM users"):
            ...
            # Check if this password is currently connected
            password = r["password"]
            is_online = active_connections.get(password, False)
            
            is_multi_login = password in multi_login_passwords
            rows.append({
...
def sync():
    ...
    # Use reload instead of restart to maintain existing connections
    try:
        subprocess.Popen(["systemctl","reload",ZIVPN_SVC], stdout=DEVNULL, stderr=DEVNULL)
    except Exception:
        # Fallback to HUP signal if reload fails
        try:
            result = subprocess.check_output(["systemctl","show","-p","MainPID",ZIVPN_SVC]).decode().strip()
            main_pid = result.split("=")[1]
            if main_pid and main_pid != "0":
                subprocess.Popen(["kill","-HUP",main_pid], stdout=DEVNULL, stderr=DEVNULL)
        except Exception:
            pass
...
def api_status():
    ...
    status_data = {
        'users': [],
        'total_online': sum(1 for r in rows if r['online'] and not r['expired']),
    ...
    for r in rows:
        status_data['users'].append({
            ...
            'online': r['online'],
            'expired': r['expired'],
            'is_multi_login': r['is_multi_login']
        })
...
function updateUserStatus(users) {
    users.forEach(user => {
        ...
        if (statusElement) {
            if (user.expired) {
                statusElement.innerHTML = '<span class="inline-flex items-center gap-1 text-slate-600"><span class="w-2 h-2 rounded-full bg-slate-400"></span>Offline</span>';
            } else if (user.online) {
                statusElement.innerHTML = '<span class="inline-flex items-center gap-1 text-emerald-700"><span class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>Online</span>';
            } else {
                statusElement.innerHTML = '<span class="inline-flex items-center gap-1 text-slate-600"><span class="w-2 h-2 rounded-full bg-slate-400"></span>Offline</span>';
            }
        }
        
        // Update password background color for multi-login
        const passwordElement = document.getElementById(`password-${user.id}`);
        if (passwordElement) {
            if (user.is_multi_login) {
                passwordElement.className = 'code-chip px-1.5 py-0.5 bg-rose-100 text-rose-700 rounded truncate-soft';
            } else {
                passwordElement.className = 'code-chip px-1.5 py-0.5 bg-slate-100 rounded truncate-soft';
            }
        }
    });
}
...
<header class="bg-gradient-to-r from-slate-900 to-slate-800 text-white">
  <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
    ...
    <div class="text-sm">
      <span class="text-emerald-400">Real-time Status</span>
    </div>
  </div>
</header>
...
{{ msg }}
  ...
  <section class="grid grid-cols-1 sm:grid-cols-4 gap-3">
    ...
    <div class="bg-white rounded-2xl shadow p-4 ring-1 ring-slate-200">
      <div class="text-slate-500 text-xs">Online Now</div>
      <div id="online-count" class="mt-1 text-2xl font-bold text-emerald-600">{{total_online}}</div>
    </div>
    <div class="bg-white rounded-2xl shadow p-4 ring-1 ring-slate-200">
      <div class="text-slate-500 text-xs">Offline</div>
      <div class="mt-1 text-2xl font-bold text-amber-600">{{total_offline}}</div>
    </div>
    {% if total_expired > 0 %}
    <div class="bg-white rounded-2xl shadow p-4 ring-1 ring-slate-200">
  ...
  <section class="grid md:grid-cols-[320px_1fr] gap-3">
    <div class="bg-white p-3 rounded-2xl shadow ring-1 ring-slate-200">
      ...
      <div class="mt-3 p-2 bg-blue-50 rounded-lg">
        <p class="text-[11px] text-blue-700">
          <span class="font-bold">Real-time Status:</span><br>
          • Online users show green pulsing dot<br>
          • Multi-login passwords show in red<br>
    ...
    <div class="bg-white p-3 rounded-2xl shadow ring-1 ring-slate-200 overflow-x-auto">
      <table class="w-full text-left align-middle table-tight">
          ...
          <tr class="border-t">
            ...
            <td class="py-1">
              <div class="flex items-center gap-1.5 flex-wrap">
                <code id="password-{{r['id']}}" class="code-chip px-1.5 py-0.5 {% if r['is_multi_login'] %}bg-rose-100 text-rose-700{% else %}bg-slate-100{% endif %} rounded truncate-soft" title="{{r['password']}}">
                  {{r['password']}}
                </code>
                <button onclick="copyText('{{r['password']}}',this)" class="btn-slim bg-slate-800 text-white rounded text-[11px]">Copy</button>
                {% if r['is_multi_login'] %}
                <span class="badge bg-rose-100 text-rose-700">Multi</span>
                {% endif %}
            ...
            <td class="py-1" id="status-{{r['id']}}">
              {% if r['expired'] %}
                <span class="inline-flex items-center gap-1 text-slate-600"><span class="w-2 h-2 rounded-full bg-slate-400"></span>Offline</span>
              {% elif r['online'] %}
                <span class="inline-flex items-center gap-1 text-emerald-700"><span class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>Online</span>
              {% else %}
                <span class="inline-flex items-center gap-1 text-slate-600"><span class="w-2 h-2 rounded-full bg-slate-400"></span>Offline</span>
              {% endif %}
            </td>
        ...
        rows=rows, total_users=total_users, total_online=total_online, 
        total_offline=total_offline, total_expired=total_expired,
...
try:
    ...
    # Fallback to HUP signal if reload fails
    try:
        result = subprocess.check_output(["systemctl","show","-p","MainPID",SVC]).decode().strip()
        main_pid = result.split("=")[1]
        if main_pid and main_pid != "0":
            subprocess.Popen(["kill","-HUP",main_pid], stdout=DEVNULL, stderr=DEVNULL)
    except Exception:
...
echo "======================================"
echo "🎯 FEATURES:"
echo "• Real-time connection status (Online/Offline)"
echo "• Multi-login detection (Red highlighting)"
echo "• Auto-refresh every 10 seconds"
echo "• Graceful server reload (no disconnections)"
echo "• Password 1111 connected = Green Online status"
echo "• Password 1111 disconnected = Gray Offline status"
BASH
