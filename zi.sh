cat > zi.sh <<'BASH'
#!/bin/bash
# ZIVPN UDP + Web Admin (Login UI, IP display, Edit/Delete, Save+Sync, Expiry enforcement)
set -euo pipefail
...
apt-get install -y python3-venv python3-pip curl jq > /dev/null
...
# ===== Web Admin Panel (Flask + Waitress) =====
...
# --- app.py (nice login, shows IP, CRUD, Save+Sync, only non-expired passwords applied) ---
cat > "${APP_PY}" <<'PY'
#!/usr/bin/env python3
...
<style>
:root{--bg:#f6f7fb;--card:#fff;--pri:#16a34a;--danger:#dc3545;--text:#111827;--muted:#6b7280}
...
# --- sync.py (auto apply only non-expired passwords; used by systemd timer) ---
cat > "${SYNC_PY}" <<'PY'
...
# systemd oneshot + timer for daily sync (00:10)
