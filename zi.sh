cat > zi.sh <<'BASH'
#!/bin/bash
# ZIVPN UDP + Tailwind Web Panel (Restart-free; Reload-only)
# Script By: JueHtet
# Rev: Save = config write only; Apply = reload only (never restart)
...
# --- systemd unit (ExecReload only; never auto-restart from panel) ---
