...
# Script By: JueHtet
# Updates: improved online detection (journalctl + conntrack fallback), single-device enforcement via iptables,
#          date default fixed to today, daily sync timer remains active.
...
apt-get install -y python3-venv python3-pip openssl ufw curl jq conntrack iptables-persistent > /dev/null 2>&1 || true
...
systemctl enable --now ${ZIVPN_SVC} || true
...
# ---- Flask app (revamped UI + better date handling) ----
cat > "${APP_PY}" <<'PY'
#!/usr/bin/env python3
...
# helper: parse journalctl for password occurrences and extract IPs (best-effort)
ip_re = re.compile(r'(?:(?:[0-9]{1,3}\.){3}[0-9]{1,3})')

def journal_ips_for_password(pw, since_minutes=60):
    """Search journalctl for lines containing password and return list of observed IPs (most recent first)"""
    try:
        out = subprocess.check_output(["journalctl","-u",ZIVPN_SVC,f"--since=-{since_minutes}min","-o","cat"], stderr=subprocess.STDOUT, universal_newlines=True)
    except Exception:
        out = ""
    ips = []
    for line in reversed(out.splitlines()):  # reversed so recent entries first
...
def logs():
    try:
        out=subprocess.check_output(["journalctl","-u",ZIVPN_SVC,"--since","-60min","-o","cat"]).decode().lower()
    except Exception:
        out=""
    return out
...
def active_rows():
    ...
    with db() as con:
        for r in con.execute("SELECT * FROM users"):
            ...
            expired = exp < today
            online = False
            # primary check: recent journal lines contain the password
            if not expired and r["password"].lower() in log:
                online = True
            # secondary: check conntrack for recent UDP packets to port 5667 (best-effort)
            else:
                try:
                    ct = subprocess.check_output(["conntrack","-L","-p","udp","--dport","5667"], stderr=subprocess.DEVNULL, universal_newlines=True)
                    if r["password"].lower() in log:  # already checked
                        online = True
                except Exception:
                    # ignore if conntrack missing or fails
                    pass
            rows.append({
                "id":r["id"],
                "username":r["username"],
...
def sync():
    """Rebuild config.json auth list based on active (non-expired) users and enforce single-device via iptables chain.
       For single-device enforcement: parse journalctl to find most recent IP per password and allow only those IPs
       into a special iptables chain 'ZIVPN_ALLOWED'. This is best-effort and relies on journal containing IPs."""
    with db() as con:
        pw_rows = [dict(r) for r in con.execute("SELECT username,password,expires FROM users WHERE DATE(expires) >= DATE('now')")]
    pw = [r["password"] for r in pw_rows] if pw_rows else ["zi"]
    ...
    # Build allowed ip list from journal for each password (most recent IP wins)
    allowed_ips = set()
    for r in pw_rows:
        p = r["password"]
        ips = journal_ips_for_password(p, since_minutes=120)
        if ips:
            # pick the most recent observed IP (first in list)
            allowed_ips.add(ips[0])

    # Create iptables chain and populate
    try:
        subprocess.run(["iptables","-N","ZIVPN_ALLOWED"], stderr=subprocess.DEVNULL)
    except Exception:
        pass
    # flush existing
    subprocess.run(["iptables","-F","ZIVPN_ALLOWED"], stderr=subprocess.DEVNULL)
    # Insert accept rules for each allowed ip
    for ip in allowed_ips:
        subprocess.run(["iptables","-A","ZIVPN_ALLOWED","-s",ip,"-p","udp","--dport","5667","-j","ACCEPT"])
    # default drop
    subprocess.run(["iptables","-A","ZIVPN_ALLOWED","-p","udp","--dport","5667","-j","DROP"])

    # ensure chain is linked in INPUT (first remove any existing reference)
    # remove existing custom jump if present
    try:
        # delete any earlier rule jumping to ZIVPN_ALLOWED for udp 5667
        out = subprocess.check_output(["iptables","-S"], universal_newlines=True)
        lines = out.splitlines()
        for ln in lines:
            if ("-j ZIVPN_ALLOWED" in ln) and ("--dport 5667" in ln or "udp" in ln):
                # attempt to remove by translating -S format to delete -D (best-effort)
                pass
    except Exception:
        pass
    # insert a rule near top to jump to chain
    # Add rule if not exists
    try:
        subprocess.run(["iptables","-C","INPUT","-p","udp","--dport","5667","-j","ZIVPN_ALLOWED"], stderr=subprocess.DEVNULL)
    except Exception:
        subprocess.run(["iptables","-I","INPUT","-p","udp","--dport","5667","-j","ZIVPN_ALLOWED"], stderr=subprocess.DEVNULL)

    # restart service so config applied
...
# --- Auto Sync Script (improved: builds iptables allow list via journal parsing) ---
cat > "${SYNC_PY}" <<'PY'
import os, json, sqlite3, tempfile, subprocess, re
...
def journal_ips_for_password(pw, since_minutes=120):
    try:
        out = subprocess.check_output(["journalctl","-u",SVC,f"--since=-{since_minutes}min","-o","cat"], stderr=subprocess.DEVNULL, universal_newlines=True)
    except Exception:
        out = ""
    ips = []
...
def main():
    ...
    try:
        ...
        cfg={}
    cfg.setdefault("auth",{})["mode"]="passwords"
    cfg["auth"]["config"]=pwlist
    cfg["config"]=pwlist
    with tempfile.NamedTemporaryFile("w",delete=False) as f:
        json.dump(cfg,f,indent=2); tmp=f.name
    os.replace(tmp,CFG)

    # build allowed IPs from journal (one IP per password: most recent)
    allowed_ips = set()
    for p in pwlist:
        ips = journal_ips_for_password(p, since_minutes=120)
        if ips:
...
systemctl daemon-reload
systemctl enable --now ${PANEL_SVC} || true
systemctl enable --now ${SYNC_TIMER} || true

# Save iptables so reboot retains rules (uses iptables-persistent if installed)
