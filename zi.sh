...
/* >>> Added: fixed table layout & column widths (consistent row/column alignment) */
...
/* Nice alignment inside cells */
.tiny td { vertical-align: middle; }
.code-area{display:flex;align-items:center;gap:.5rem;overflow:hidden}
.code-chip{display:inline-block;max-width:140px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
  ...
  <section class="grid md:grid-cols-[320px_1fr] gap-3">
    ...
    <div class="bg-white p-3 rounded-2xl shadow ring-1 ring-slate-200 overflow-x-auto">
      <table class="w-full text-left align-middle table-tight">
        <!-- >>> Added colgroup to lock widths exactly like requested -->
        <colgroup>
          <col style="width:20%">
          <col style="width:35%">
          <col style="width:18%">
          ...
          <tr class="border-t">
            ...
            <td class="py-1">
              <div class="code-area">
                ...
                <button onclick="copyText('{{r['password']}}',this)" class="btn-small bg-slate-800 text-white">Copy</button>
            ...
            <td class="py-1">
              <div class="flex items-center gap-1.5 justify-center">
                <button type="button" onclick="fillForm('{{r['username']}}','{{r['password']}}','{{r['expires']}}')" class="btn-small bg-amber-500 hover:bg-amber-400 text-white">Edit</button>
                <form method=post action="/del/{{r['id']}}" onsubmit="return confirm('Delete {{r['username']}} ?')">
                  <button class="btn-small bg-rose-600 hover:bg-rose-500 text-white">🗑️</button>
                </form>
              </div>
            </td>
          </tr>
