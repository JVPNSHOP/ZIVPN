...
# Script By: JueHtet
# Rev: No-restart-on-save (only rewrite config.json)
...
Restart=always
User=root
# If zivpn supports HUP reload later, you can enable this and call: systemctl reload zivpn
...
def sync():
    ...
    if not pw: pw=["zi"]
    # Only write config; DO NOT restart the service to avoid dropping connections.
    write_cfg(pw)
    # If later supported: subprocess.Popen(["systemctl","reload",ZIVPN_SVC], stdout=DEVNULL, stderr=DEVNULL)
...
<div class="fab">
  <a href="https://t.me/Pussy1990" target="_blank" rel="noopener" class="bg-sky-600 hover:bg-sky-500 text-white" title="Telegram">
    <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M9.04 15.47l-.39 5.49c.56 0 .8-.24 1.09-.53l2.62-2.52 5.43 3.97c1 .55 1.71.26 1.98-.93લ3.6-16.85c.32-1.5-.54-2.09-1.52-1.73L1.16 9.64c-1.46.57-1.44 1.39-.25 1.76l5.34 1.66L19.36 6.1c.62-.41 1.18-.18.72.23"/></svg>
  </a>
    ...
    <div class="bg-white p-3 rounded-2xl shadow ring-1 ring-slate-200">
      ...
      <form method=post action="/save" class="space-y-2">
        <div class="relative">
          ...
          <input name=username placeholder="👤 Username" class="w-full pl-10 border rounded-lg p-2 text-sm focus:ring-2 focus:ring-emerald-500 outline-none">
        </div>
        <div class="relative">
          <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <input name=password placeholder="🔒 Password" class="w-full pl-10 border rounded-lg p-2 text-sm focus:ring-2 focus:ring-emerald-500 outline-none">
        </div>
...
if __name__=="__main__":
    write_cfg(actives())
    # Intentionally do NOT restart service here (no connection drop).
PY
