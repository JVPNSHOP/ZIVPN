# (အပေါင်းအစ) — zi.sh စာလုံးကြီးလုံးဝပြင်ထားချက်
# copy the entire content into a file named zi.sh and run chmod +x zi.sh

cat > zi.sh <<'BASH'
#!/bin/bash
# ZIVPN UDP + Tailwind Web Panel (Hardened)
# - Save = config.json write only (no reload/restart) - FIXED
# - Apply = reload only (never restart) - FIXED  
# - UDP stability: conntrack timeouts ++ & DNAT->REDIRECT
# Script By: JueHtet | Connection Stability Fix by: helper
...
iptables -t nat -S PREROUTING 2>/dev/null | awk '/--dport 6000:19999/ {print $0}' | sed 's/^-A /-D /' | while read -r r; do iptables -t nat $r || true; done
# Add REDIRECT
iptables -t nat -A PREROUTING -i "$IFC" -p udp --dport 6000:19999 -j REDIRECT --to-ports 5667 2>/dev/null || true
...
# FIXED: User အသစ်ထည့်တိုင်း Connection မပြတ်အောင် ပြင်ဆင်ထားသည်
cat > "${APP_PY}" <<'PY'
#!/usr/bin/env python3
...
def sync():
    # FIXED: Save-time: only write config (no reload/restart) - Connection မပြတ်တော့ပါ
    with db() as con:
...
def index():
    ...
    try:
        service_status = subprocess.call(["systemctl", "is-active", ZIVPN_SVC], stdout=DEVNULL, stderr=DEVNULL) == 0
    except:
...
.tiny{font-size:13px;line-height:1.2}
.code-chip{font-family:ui-monospace,Menlo,monospace; display:inline-block; max-width:140px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; vertical-align:middle}
...
/* New: force table column widths and align rows */
table { table-layout: fixed; width: 100%; }
.table-tight th:nth-child(1), .table-tight td:nth-child(1) { width: 20%; min-width:120px; }
.table-tight th:nth-child(2), .table-tight td:nth-child(2) { width: 35%; min-width:200px; }
.table-tight th:nth-child(3), .table-tight td:nth-child(3) { width: 18%; min-width:120px; }
.table-tight th:nth-child(4), .table-tight td:nth-child(4) { width: 15%; min-width:90px; text-align:left; }
.table-tight th:nth-child(5), .table-tight td:nth-child(5) { width: 12%; min-width:90px; text-align:center; }
.tiny td { vertical-align: middle; }
.btn-small { font-size:12px; padding:.25rem .5rem; border-radius:.5rem; }
.code-area { display:flex; align-items:center; gap:.5rem; overflow:hidden; }
</style>
</head>
...
<header class="bg-gradient-to-r from-slate-900 to-slate-800 text-white">
  <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
    <div class="flex items-center gap-2">
      <img src="https://raw.githubusercontent.com/JVPNSHOP/ZIVPN/main/1761213901286.png" alt="Logo" class="h-8 w-8">
      <h1 class="text-2xl font-extrabold tracking-tight">ZIVPN - FIXED</h1>
    </div>
    <div class="flex items-center gap-2">
      {% if service_status %}
        <span class="bg-green-500 text-white px-2 py-1 rounded text-sm">🟢 Service Running</span>
      {% else %}
        <span class="bg-red-500 text-white px-2 py-1 rounded text-sm">🔴 Service Stopped</span>
      {% endif %}
    </div>
  </div>
</header>
...
<main class="max-w-6xl mx-auto px-4 py-4 space-y-4">
  ...
  <section class="bg-white rounded-2xl shadow p-4 ring-1 ring-slate-200">
    ...
    <div class="mt-2 p-2 bg-green-50 rounded-lg">
      <p class="text-green-700 text-xs">✅ FIXED: User အသစ်ထည့်ရင် Connection မပြတ်တော့ပါ</p>
    </div>
  </section>
...
{{ msg }}
      ...
      <div class="mt-1 text-[11px] text-emerald-800/80">1 User For 1 Device | Connection Stable 🔒</div>
    </div>
    {% endfor %}
  {% endif %}
  {% endwith %}
  ...
  <section class="flex gap-2">
    <form method="post" action="/apply">
      <button class="bg-green-600 hover:bg-green-500 text-white rounded-xl px-3 py-2 text-sm">⚙️ Apply Config (Safe Reload)</button>
    </form>
    <form method="post" action="/restart">
      <button class="bg-amber-600 hover:bg-amber-500 text-white rounded-xl px-3 py-2 text-sm" onclick="return confirm('Restart service? Existing connections may drop.')">🔄 Restart Service</button>
    </form>
  </section>
  ...
  <section class="grid md:grid-cols-[320px_1fr] gap-3">
    <div class="bg-white p-3 rounded-2xl shadow ring-1 ring-slate-200">
      ...
      <form method=post action="/save" class="space-y-2">
        ...
        <button class="w-full bg-emerald-600 hover:bg-emerald-500 text-white py-2 rounded-xl shadow text-sm">💾 Save & Sync (No Restart)</button>
      </form>
      <p class="mt-2 text-[11px] text-slate-500">Script By: <b>JueHtet</b> | Fixed by: <b>helper</b></p>
      <p class="text-[10px] text-green-600">✅ Connection Stable Version</p>
    </div>
      ...
      <table class="w-full text-left align-middle table-tight">
        ...
        <tbody class="tiny">
          ...
          <tr class="border-t">
            ...
            <td class="py-1">
              <div class="code-area">
                <code class="code-chip bg-slate-100 px-1.5 py-0.5 rounded" title="{{r['password']}}">{{r['password']}}</code>
                <button onclick="copyText('{{r['password']}}',this)" class="btn-small bg-slate-800 text-white rounded">Copy</button>
            ...
            <td class="py-1">
              <div class="flex items-center gap-1.5 justify-center">
                <button type="button" onclick="fillForm('{{r['username']}}','{{r['password']}}','{{r['expires']}}')" class="btn-small bg-amber-500 hover:bg-amber-400 text-white rounded">Edit</button>
                <form method=post action="/del/{{r['id']}}" onsubmit="return confirm('Delete {{r['username']}} ?')">
                  <button class="btn-small bg-rose-600 hover:bg-rose-500 text-white rounded">🗑️</button>
                </form>
              </div>
            </td>
          </tr>
        ...
        default_exp=default_exp, vps_ip=vps_ip, server_ts=server_ts, service_status=service_status)
...
def apply():
    try:
        ...
        sync()
        # Then safe reload
        result = subprocess.run(["systemctl","reload",ZIVPN_SVC], capture_output=True, text=True, timeout=10)
        if result.returncode == 0:
            flash("✅ Config applied with SAFE RELOAD - Connections preserved!", "ok")
        else:
            flash("⚠️ Config updated but reload failed. Changes will take effect on next connection.", "ok")
    except subprocess.TimeoutExpired:
        flash("⚠️ Reload timed out but config was updated. Connections preserved.", "ok")
...
def restart():
    try:
        subprocess.run(["systemctl","restart",ZIVPN_SVC], timeout=10)
        flash("Service restarted. Existing connections may have dropped.", "ok")
    except Exception as e:
        flash(f"Restart failed: {str(e)}", "err")
    return redirect("/")
...
[Unit]
Description=ZIVPN Web Panel (Fixed - Connection Stable)
After=network.target
[Service]
...
[Unit]
Description=ZIVPN Daily Sync (No Restart)
[Service]
Type=oneshot
...
echo
echo "✅ INSTALL COMPLETE - CONNECTION STABLE VERSION"
echo "================================================"
...
echo "================================================"
echo "🔧 FIXED FEATURES:"
echo "   ✅ User အသစ်ထည့်ရင် Connection မပြတ်တော့ပါ"
echo "   ✅ Save = Config ပြင်တယ် (Restart မလုပ်ဘူး)"
echo "   ✅ Apply = Safe Reload လုပ်တယ် (Restart မလုပ်ဘူး)"
echo "   ✅ Existing connections တွေ ဆက်အလုပ်လုပ်နေမယ်"
echo "================================================"
BASH

chmod +x zi.sh
echo "✅ Fixed ZIVPN Script Ready!"
echo "🔧 Run with: sudo ./zi.sh"
