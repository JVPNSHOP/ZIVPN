...
Restart=always
User=root
# Define ExecReload to support graceful config reload (HUP signal)
...
def get_online_users():
    ...
    try:
        # Check logs from the last 1 minute (should be sufficient for a 20s timer)
...
def active_rows():
    ...
    if monitor_users(online_counts): 
        # If someone was expired, run a sync/reload immediately
        sync()
...
def sync():
    ...
    # Use systemctl reload for less disruption
...
os.replace(tmp,CFG)
# Use systemctl reload instead of restart for less disruption
...
def monitor_and_expire():
    ...
    if expired_count > 0:
        con.commit()
        # If any user was expired, run sync to update ZIVPN config immediately
        run_sync()
