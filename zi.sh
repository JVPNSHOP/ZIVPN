...
# Tweaks: copy fix, remove "Copy Details", 1-device policy => require admin re-enable
# Script By: JueHtet
          ...
          <tr class="border-t">
            ...
            <td class="py-1">
              <div class="flex items-center gap-1.5 flex-wrap">
                ...
                <button type="button" onclick="return copyText({{ r['password']|tojson }}, this)" class="btn-slim bg-slate-800 text-white rounded text-[11px]">Copy</button>
...
# --- 1-device-per-password Guard (require admin to re-enable) ---
cat > "${GUARD_PY}" <<'PY'
    ...
    while True:
        ...
        with sqlite3.connect(DB) as con:
            ...
            for uid,uname,pw in users():
                ...
                if len(ips) > 1:
                    # Disable account (expire yesterday) -> must contact admin to re-enable
