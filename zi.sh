cat > zi-fixed.sh <<'BASH'
#!/bin/bash
# ZIVPN UDP + Tailwind Web Panel (Fixed Real-time Status & Multi-Login)
# Script By: JueHtet
...
apt-get install -y python3-venv python3-pip openssl ufw curl jq net-tools > /dev/null
...
# Create service with better logging
...
def get_active_connections():
    """Get currently active connections by checking ZIVPN logs"""
    active_passwords = set()
    
    try:
        # Get recent ZIVPN logs (last 2 minutes)
        ...
        with db() as con:
            passwords = [row[0] for row in con.execute("SELECT DISTINCT password FROM users WHERE DATE(expires) >= DATE('now')")]
        
        # Check each password in logs
        for password in passwords:
            if password and len(password) >= 4:
                # If password appears in recent logs, consider it active
                if password.lower() in logs:
                    active_passwords.add(password)
        
        # Additional check: look for connection patterns in logs
        connection_patterns = [
            r'connected',
            r'established', 
            r'success',
        ...
        for pattern in connection_patterns:
            if re.search(pattern, logs, re.IGNORECASE):
                # If we see connection patterns, check for specific passwords
                for password in passwords:
                    if password and len(password) >= 4:
                        # Simple heuristic: if password mentioned and connection patterns exist
                        if password.lower() in logs:
                            active_passwords.add(password)
...
def active_rows():
    ...
    with db() as con:
        for r in con.execute("SELECT * FROM users"):
            ...
            # Check if this password is currently active
            password = r["password"]
            is_online = password in active_connections
            
            # MULTI-LOGIN DISABLED - Allow multiple devices
...
// Auto refresh status every 3 seconds for better real-time
...
function updateOnlineCount(count) {
    ...
    if (onlineElement) {
        ...
        onlineElement.classList.add('scale-110');
        setTimeout(() => onlineElement.classList.remove('scale-110'), 300);
    }
}
...
function updateUserStatus(users) {
    users.forEach(user => {
        ...
        if (statusElement) {
            if (user.expired) {
                statusElement.innerHTML = '<span class="inline-flex items-center gap-1 text-slate-600"><span class="w-2 h-2 rounded-full bg-slate-400"></span>Expired</span>';
            } else if (user.online) {
                statusElement.innerHTML = '<span class="inline-flex items-center gap-1 text-emerald-700 font-semibold"><span class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>LIVE</span>';
            } else {
...
<header class="bg-gradient-to-r from-slate-900 to-slate-800 text-white">
  <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
    ...
    <div class="text-sm">
      <span class="text-emerald-400">Fixed Real-time Status</span>
    </div>
  </div>
</header>
...
{{ msg }}
      ...
      <div class="mt-1 text-[11px] text-emerald-800/80">Multiple devices allowed per password</div>
    </div>
    {% endfor %}
  {% endif %}
  {% endwith %}
    ...
    <div class="bg-white p-3 rounded-2xl shadow ring-1 ring-slate-200">
      ...
      <div class="mt-3 p-2 bg-green-50 rounded-lg">
        <p class="text-[11px] text-green-700">
          <span class="font-bold">Fixed Issues:</span><br>
          • Real-time status working<br>
          • Multi-login restrictions removed<br>
          • Multiple devices allowed<br>
          ...
          <tr class="border-t">
            ...
            <td class="py-1">
              <div class="flex items-center gap-1.5 flex-wrap">
                <code class="code-chip px-1.5 py-0.5 bg-slate-100 rounded truncate-soft" title="{{r['password']}}">
                  {{r['password']}}
                </code>
            ...
            <td class="py-1" id="status-{{r['id']}}">
              ...
              {% elif r['online'] %}
                <span class="inline-flex items-center gap-1 text-emerald-700 font-semibold"><span class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>LIVE</span>
              {% else %}
...
def save():
    ...
    msg=f"IP : {ip}\nUsers : {u}\nPassword : {p}\nExpired Date : {e}\nMultiple devices allowed per password"
    flash(msg, "ok")
...
# Clear existing iptables and reapply
iptables -t nat -F
IFC=$(ip -4 route ls | awk '/default/ {print $5; exit}')
iptables -t nat -A PREROUTING -i "$IFC" -p udp --dport 6000:19999 -j DNAT --to-destination :5667
...
echo "======================================"
echo "🎯 FIXED ISSUES:"
echo "• Real-time status now working properly"
echo "• Multi-login restrictions COMPLETELY REMOVED"
echo "• Multiple devices allowed per password"
echo "• Faster auto-refresh (3 seconds)"
echo "• Better connection detection"
echo ""
echo "🔧 TEST INSTRUCTIONS:"
echo "1. Create user with password 1111"
echo "2. Connect from device using password 1111"
echo "3. Panel should show LIVE status immediately"
echo "4. Multiple devices can use same password"
BASH
