...
# NAT သေချာအောင် iptables-persistent လည်းထည့်ထား
apt-get install -y python3-venv python3-pip openssl ufw curl jq iptables-persistent > /dev/null
...
# ---------------- Internet NAT / FORWARDING FIX (added) ----------------
IFC=$(ip -4 route ls | awk '/default/ {print $5; exit}')
# DNAT: port range -> 5667 (မင်းအစကဲ့သို့)
...
function copy(t,b){
  if(navigator.clipboard&&navigator.clipboard.writeText){
    navigator.clipboard.writeText(t).then(()=>{b.innerText='✓ Copied';setTimeout(()=>b.innerText='Copy',900);})
    .catch(()=>{const e=document.createElement('textarea');e.value=t;document.body.appendChild(e);e.select();document.execCommand('copy');document.body.removeChild(e);b.innerText='✓ Copied';setTimeout(()=>b.innerText='Copy',900);});
  }else{
    const e=document.createElement('textarea');e.value=t;document.body.appendChild(e);e.select();document.execCommand('copy');document.body.removeChild(e);b.innerText='✓ Copied';setTimeout(()=>b.innerText='Copy',900);
  }
}
</script>
</head>
