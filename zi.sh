#!/bin/bash
# All-in-one installer: ZIVPN + Web Admin Panel
# Creator: Zahid Islam (base UDP installer) + web panel glue
...
# ---------- Install zivpn ----------
echo "==> Installing ZIVPN UDP service"
systemctl stop "${ZIVPN_SVC}" 1>/dev/null 2>/dev/null || true
...
# Safely update config.json's "config" array using Python (preserves other fields)
python3 - "$ZIVPN_CFG" "$input_config" <<'PY'
import sys, json
cfg_path, csv = sys.argv[1], sys.argv[2]
pw = [x.strip() for x in csv.split(",") if x.strip()]
if not pw: pw = ["zi"]
try:
...
systemctl enable "${ZIVPN_SVC}"
systemctl start "${ZIVPN_SVC}"

# one-time firewall/forwarding (same as original)
echo "==> Setting firewall rules"
IFC=$(ip -4 route ls | grep default | grep -Po '(?<=dev )(\S+)' | head -1)
...
echo "==> Installing Web Admin Panel (Flask)"
...
# Write the Flask panel (single file)
cat > "${APP_PY}" <<'PY'
#!/usr/bin/env python3
# ZIVPN Web Admin Panel (single-file)
...
def sync_config_and_restart():
    ...
    # write atomically (fallback to sudo tee)
    try:
        ...
        import subprocess
        p = subprocess.run(["sudo","tee",ZIVPN_CONFIG_PATH], input=text.encode(), stdout=subprocess.DEVNULL)
        if p.returncode!=0:
            raise
    subprocess.run(["sudo","systemctl","restart",ZIVPN_SERVICE], check=False)

@app.route("/", methods=["GET"]); @require_auth
def index():
...
@app.route("/save", methods=["POST"]); @require_auth
def save():
...
@app.route("/delete/<int:user_id>", methods=["POST"]); @require_auth
def delete(user_id):
    delete_user(user_id); flash("Deleted"); return redirect(url_for("index"))

@app.route("/sync", methods=["POST"]); @require_auth
def sync():
    try: sync_config_and_restart(); flash("Synced to config.json + restarted service")
...
echo "Admin Panel svc   : ${PANEL_SVC} (systemctl status ${PANEL_SVC})"
echo
echo ">> Access the Web Panel (recommended via SSH tunnel):"
echo "   On your PC: ssh -L ${BIND_PORT}:127.0.0.1:${BIND_PORT} <user>@${IP}"
echo "   Then open:  http://127.0.0.1:${BIND_PORT}/"
echo "   Login user/password: ${ADMIN_USER}/${ADMIN_PASSWORD}"
echo "========================================"
