...
# Script By: JueHtet (edited: remove restarts + replace notice text)
...
# Port map: 6000-19999/udp -> 5667/udp
...
def sync():
    # build password list (no server restart here)
    with db() as con:
    ...
    # IMPORTANT: no automatic restart/reload to avoid cutting active sessions
...
<div class="fab">
  ...
  <a href="/logout" class="bg-slate-700 hover:bg-slate-600 text-white" title="Logout">
    <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M10 17v2H5a2 2 0 0 1-2-2V7a2 2 0 0 1 2-2h5v2H5v10h5zm4.293-1.293 2.293-2.293H9v-2h7.586l-2.293-2.293 1.414-1.414z"/></svg>
  </a>
</div>
...
{{ msg }}
      ...
      <div class="mt-1 text-[11px] text-emerald-800/80">Script For Sale</div>
    </div>
    {% endfor %}
  {% endif %}
  {% endwith %}
...
with tempfile.NamedTemporaryFile("w",delete=False) as f:
    json.dump(cfg,f,indent=2); tmp=f.name
os.replace(tmp,CFG)
# NOTE: intentionally no restart/reload to avoid session cuts
PY
