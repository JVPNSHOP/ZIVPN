cat > zi.sh <<'BASH'
#!/bin/bash
# ZIVPN UDP + Admin Panel + HARD Internet NAT/FORWARDING
# Script By: JueHtet — v6 Internet-Force (keeps your UI/Copy/Edit/Renew & Online-by-expiry)
...
# ------------------------ INTERNET FIX (HARD MODE) ------------------------
say "Detect outbound interface..."
IFC=$(ip -4 route ls | awk '/default/ {print $5; exit}')
if [ -z "${IFC:-}" ]; then
  echo "Cannot detect outbound interface (default route). Abort."; exit 1
fi
say "Outbound = $IFC"

say "Enable IPv4 forwarding (runtime + persist)..."
sysctl -w net.ipv4.ip_forward=1 >/dev/null
echo "net.ipv4.ip_forward=1" > /etc/sysctl.d/90-zivpn.conf
sysctl --system >/dev/null

# Defensive sysctl for NAT environments
echo "net.ipv4.conf.all.rp_filter=0"  > /etc/sysctl.d/91-zivpn-rpf.conf
echo "net.ipv4.conf.default.rp_filter=0" >> /etc/sysctl.d/91-zivpn-rpf.conf
sysctl --system >/dev/null

say "Insert iptables rules (top of chains)..."
# DNAT: 6000-19999 -> 5667 (place at top)
iptables -t nat -C PREROUTING -i "$IFC" -p udp --dport 6000:19999 -j DNAT --to-destination :5667 2>/dev/null || \
iptables -t nat -I PREROUTING 1 -i "$IFC" -p udp --dport 6000:19999 -j DNAT --to-destination :5667

# MASQUERADE: all egress via IFC
iptables -t nat -C POSTROUTING -o "$IFC" -j MASQUERADE 2>/dev/null || \
iptables -t nat -I POSTROUTING 1 -o "$IFC" -j MASQUERADE

# FORWARD policy & rules
iptables -P FORWARD ACCEPT || true
iptables -C FORWARD -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT 2>/dev/null || \
iptables -I FORWARD 1 -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT
iptables -C FORWARD -j ACCEPT 2>/dev/null || iptables -I FORWARD 2 -j ACCEPT

# MSS clamp to prevent MTU blackholes (TCP only)
iptables -t mangle -C FORWARD -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --clamp-mss-to-pmtu 2>/dev/null || \
iptables -t mangle -I FORWARD 1 -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --clamp-mss-to-pmtu

# Persist iptables if available
if command -v netfilter-persistent >/dev/null 2>&1; then
  netfilter-persistent save || true
fi

say "Adjust UFW if active..."
if ufw status | grep -q "Status: active"; then
  ufw allow 8088/tcp || true
  ufw allow 5667/udp || true
  sed -i 's/^DEFAULT_FORWARD_POLICY=.*/DEFAULT_FORWARD_POLICY="ACCEPT"/' /etc/default/ufw

  # Ensure NAT block exists in before.rules
  if ! grep -q "ZIVPN NAT BLOCK" /etc/ufw/before.rules 2>/dev/null; then
...
EOFUFW
  ...
  # UFW sysctls to enable forward
  sed -i 's/^#\?net\/ipv4\/ip_forward=.*/net\/ipv4\/ip_forward=1/' /etc/ufw/sysctl.conf
  ufw reload
fi
# ---------------------- END INTERNET FIX ----------------------

say "Setup Admin Panel (keeps your UI/features)..."
mkdir -p "${ADMIN_DIR}"
python3 -m venv "${VENV}"
...
def active_passwords():
    ...
    with sqlite3.connect(DB) as con:
        rows=[r[0] for r in con.execute("SELECT DISTINCT password FROM users WHERE DATE(expires)>=DATE('now')")]
        return rows or ["zi"]
pw=active_passwords()
cfg={"listen":":5667","auth":{"mode":"passwords","config":pw},"config":pw}
