cat > zi.sh <<'BASH'
#!/bin/bash
# ZIVPN UDP + Tailwind Web Panel (UI-only tweaks per request)
# Script By: JueHtet
...
# ---- Flask app (UI revamp per request) ----
cat > "${APP_PY}" <<'PY'
#!/usr/bin/env python3
...
<script>
// time ticker from server epoch
const SERVER_TS={{server_ts}}*1000;
let start=Date.now();
...
// copy + picker + flash helpers
function copyText(t, btn){
  function ok(){ if(btn){ btn.innerText='✓ Copied'; btn.disabled=true; setTimeout(()=>{btn.innerText='Copy URL'; btn.disabled=false;},900);} }
  if (navigator.clipboard && window.isSecureContext){
    navigator.clipboard.writeText(t).then(ok).catch(()=>fallback());
  } else { fallback(); }
  function fallback(){ const ta=document.createElement('textarea'); ta.value=t; ta.style.position='fixed'; ta.style.opacity='0'; document.body.appendChild(ta); ta.focus(); ta.select(); try{ document.execCommand('copy'); ok(); }catch(e){} document.body.removeChild(ta); }
}
function fillForm(u,p,e){
  const f=document.querySelector('form[action="/save"]'); if(!f) return;
  const iu=f.querySelector('input[name="username"]'); const ip=f.querySelector('input[name="password"]'); const ie=f.querySelector('input[name="expires"]');
  iu.value=u; ip.value=p; ie.value=e; f.scrollIntoView({behavior:'smooth', block:'start'});
  setTimeout(()=>{ ie.removeAttribute('readonly'); ie.focus({preventScroll:true}); if(ie.showPicker) ie.showPicker(); },150);
}
...
<header class="bg-gradient-to-r from-slate-900 to-slate-800 text-white">
  <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
    ...
    <div class="flex items-center gap-2">
      <!-- rounded info pill: VPS IP + Server Time -->
      <span class="text-[11px] bg-emerald-600/20 ring-1 ring-emerald-400/40 px-3 py-1 rounded-full">
        VPS IP: <b>{{ vps_ip }}</b> • Server Time: <b id="server-time">--:--:--</b>
      </span>
      <!-- Telegram link with icon -->
      <a href="https://t.me/Pussy1990" target="_blank" rel="noopener" class="btn-slim bg-sky-600 hover:bg-sky-500 rounded text-[11px] flex items-center gap-1">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M9.04 15.47l-.39 5.49c.56 0 .8-.24 1.09-.53l2.62-2.52 5.43 3.97c1 .55 1.71.26 1.98-.93l3.6-16.85.01-.01c.32-1.5-.54-2.09-1.52-1.73L1.16 9.64c-1.46.57-1.44 1.39-.25 1.76l5.34 1.66L19.36 6.1c.62-.41 1.18-.18.72.23"/></svg>
        Telegram
      </a>
      <!-- Copy URL -->
      ...
      <!-- Logout small button -->
      <a href="/logout" class="btn-slim bg-slate-700 hover:bg-slate-600 rounded text-[11px] text-white">Logout</a>
    </div>
  </div>
</header>
...
{{ msg }}
      ...
      <div class="mt-2 flex gap-2">
        <button class="btn-slim bg-slate-800 text-white rounded text-[11px]" onclick="copyText('{{ msg|replace('\n',' \\n') }}', this)">Copy Details</button>
      </div>
  ...
  <!-- Stats Cards (subtitle removed as requested) -->
  ...
  <!-- Forms + Table -->
  <section class="grid md-grid-cols-[320px_1fr] md:grid-cols-[320px_1fr] gap-3">
    <div class="bg-white p-3 rounded-2xl shadow ring-1 ring-slate-200">
      <h3 class="font-semibold mb-2 text-sm flex items-center gap-2">
        <!-- person icon on form heading too -->
