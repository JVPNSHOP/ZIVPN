...
# Plus: ip_forward + NAT MASQUERADE + UFW rules + persist netfilter
set -euo pipefail
...
apt-get install -y python3-venv python3-pip openssl ufw curl jq netfilter-persistent iptables-persistent >/dev/null || true
...
cat >/etc/systemd/system/${ZIVPN_SVC} <<'EOF2'
[Unit]
...
# ===== Network: IP forwarding + NAT + Firewall =====
echo "==> Enabling IP forwarding..."
echo 'net.ipv4.ip_forward=1' >/etc/sysctl.d/99-zivpn.conf
sysctl -p /etc/sysctl.d/99-zivpn.conf >/dev/null

echo "==> Adding NAT (DNAT + MASQUERADE) rules..."
IFC=$(ip -4 route ls | awk '/default/ {print $5; exit}')
# DNAT: external UDP 6000-19999 -> internal 5667
...
<header class="bg-gradient-to-r from-slate-900 to-slate-800 text-white"><div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between"><div class="flex items-center gap-2"><img src="https://raw.githubusercontent.com/JVPNSHOP/ZIVPN/main/1761213901286.png" alt="ZIVPN" class="h-7 w-7 rounded"><h1 class="text-2xl font-extrabold tracking-tight">ZIVPN</h1></div><div></div></div></header>
<main class="max-w-6xl mx-auto px-4 py-4 space-y-4">
  ...
  <div id="notice-{{ loop.index }}" class="bg-emerald-50 ring-1 ring-emerald-200 text-emerald-900 rounded-2xl p-3"><div class="flex items-start justify-between gap-2"><div class="text-sm whitespace-pre-wrap font-medium">
Done ✅
{{ msg }}
        </div><button class="btn-slim bg-emerald-600 text-white rounded text-[11px]" onclick="closeNotice('notice-{{ loop.index }}')">OK</button></div></div>
  ...
  <section class="grid grid-cols-1 sm:grid-cols-3 gap-3">
    ...
    {% if total_expired > 0 %}<div class="bg-white rounded-2xl shadow p-4 ring-1 ring-slate-200"><div class="text-slate-500 text-xs">Expired</div><div class="mt-1 text-2xl font-bold text-rose-600">{{total_expired}}</div></div>{% endif %}
  </section>
  <section class="grid md:grid-cols-[320px_1fr] gap-3">
    <div class="bg-white p-3 rounded-2xl shadow ring-1 ring-slate-200">
      <h3 class="font-semibold mb-2 text-sm flex items-center gap-2"><img src="https://raw.githubusercontent.com/JVPNSHOP/ZIVPN/main/1761213901286.png" alt="ZIVPN" class="h-4 w-4 rounded">Add / Update User</h3>
...
# ---- Panel service ----
cat >/etc/systemd/system/${PANEL_SVC} <<EOF4
[Unit]
...
cat >/etc/systemd/system/${SYNC_SVC} <<EOF5
[Unit]
...
cat >/etc/systemd/system/${SYNC_TIMER} <<'EOF6'
[Unit]
...
iptables -t nat -S | grep -E 'PREROUTING|POSTROUTING' || true
echo "======================================"
EOF

# (Windows line endings ကုန်ချင်ရင်)
sed -i 's/\r$//' zi.sh
