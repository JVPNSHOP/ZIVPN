cat > zi.sh <<'BASH'
#!/bin/bash
# ZIVPN UDP Server + Tailwind Web Panel (UI + stats + auto-refresh)
# Script By: JueHtet — password-based where possible + UDP activity fallback
...
DEBIAN_FRONTEND=noninteractive apt-get install -yq python3-venv python3-pip openssl ufw curl jq conntrack
...
def get_server_ip():
    pub=subprocess.getoutput("curl -s https://api.ipify.org")
    if pub.strip():
        return pub.strip()
    return subprocess.getoutput("hostname -I | awk '{print $1}'").strip()

def logs_window(minutes=15):
    try:
        out=subprocess.check_output(
            ["journalctl","-u",ZIVPN_SVC,"--since",f"-{minutes}min","-o","cat"],
...
def any_udp_activity():
    """Return True if any UDP flow to port 5667 exists (conntrack)."""
    try:
        out = subprocess.check_output(
            ["conntrack","-L","-p","udp","--dport","5667"],
            stderr=subprocess.DEVNULL
...
def active_rows():
    ...
    with db() as con:
        ...
        active_users=[u for u in users if datetime.strptime(u["expires"],"%Y-%m-%d").date() >= today]
        single_active=(len(active_users)==1)
        udp_live=any_udp_activity()
        for r in users:
            exp=datetime.strptime(r["expires"],"%Y-%m-%d").date()
            expired = exp < today
            # Primary (if server logs contain password)
            online = (not expired) and (r["password"].lower() in log)
            # Fallback: if only one active user exists and UDP activity is present -> mark online
            if (not online) and (not expired) and single_active and udp_live:
                online = True
            rows.append({
                "id":r["id"], "username":r["username"], "password":r["password"],
                "expires":r["expires"], "expired":expired, "online":online,
                "days":days_left(r["expires"])
            })
    return rows

def sync():
    with db() as con:
        active_pw=[r[0] for r in con.execute("SELECT DISTINCT password FROM users WHERE DATE(expires)>=DATE('now')")]
    if not active_pw: active_pw=["zi"]
    cfg={}
    try: cfg=json.load(open(ZIVPN_CFG))
    except Exception: cfg={"listen":":5667","auth":{}}
    cfg.setdefault("auth",{})["mode"]="passwords"
    cfg["auth"]["config"]=active_pw
    cfg["config"]=active_pw
...
<div class="bg-slate-800/80 p-6 rounded-2xl w-[360px]">
<h2 class="text-xl font-bold mb-3">🛡️ ZIVPN Login</h2>
<form method=post class="space-y-3">
<input name=u class="w-full p-2 rounded bg-slate-700" placeholder="Username">
<input name=p type=password class="w-full p-2 rounded bg-slate-700" placeholder="Password">
...
def index():
    ...
    default_exp=date.today().isoformat()  # default = today
    server_ip=get_server_ip()
    return render_template_string('''<!doctype html>
<html>
<head>
  <meta charset="utf-8">
  <meta http-equiv="refresh" content="20"> <!-- auto-refresh every 20s -->
...
<a href="/logout" class="fixed bottom-4 right-4 bg-sky-600 hover:bg-sky-500 text-white px-4 py-3 rounded-full shadow">Logout</a>

<header class="bg-gradient-to-r from-slate-900 to-slate-800 text-white">
  <div class="max-w-6xl mx-auto px-4 py-5">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-extrabold">🛡️ ZIVPN Admin Panel</h1>
        <p class="text-white/70 text-sm">Fast controls • Live status • Copy-ready creds</p>
      </div>
      <div class="text-xs bg-white/10 px-3 py-1 rounded-full">Script By: <b>JueHtet</b></div>
    </div>
    <div class="mt-3 text-sm">Server: <span class="font-mono">{{server_ip}}</span> • Panel: <span class="font-mono">{{server_ip}}:8088</span> • UDP: 5667</div>
  </div>
</header>

<main class="max-w-6xl mx-auto px-4 py-6 space-y-6">
  <section class="grid grid-cols-1 sm:grid-cols-3 gap-4">
    <div class="bg-white rounded-2xl shadow p-5 ring-1 ring-slate-200"><div class="text-slate-500 text-sm">Total Users</div><div class="mt-1 text-3xl font-bold">{{total_users}}</div></div>
    <div class="bg-white rounded-2xl shadow p-5 ring-1 ring-slate-200"><div class="text-slate-500 text-sm">Total Online</div><div class="mt-1 text-3xl font-bold text-emerald-600">{{total_online}}</div></div>
    <div class="bg-white rounded-2xl shadow p-5 ring-1 ring-slate-200"><div class="text-slate-500 text-sm">Total Offline</div><div class="mt-1 text-3xl font-bold text-rose-600">{{total_offline}}</div></div>
  </section>
  ...
  <section class="grid md:grid-cols-[340px_1fr] gap-4">
    <div class="bg-white p-4 rounded-2xl shadow ring-1 ring-slate-200">
      ...
      <form method=post action="/save" class="space-y-2">
        ...
        <input type=date name=expires value="{{default_exp}}" min="{{default_exp}}" class="w-full border rounded-lg p-2 focus:ring-2 focus:ring-emerald-500 outline-none">
    ...
    <div class="bg-white p-4 rounded-2xl shadow ring-1 ring-slate-200 overflow-x-auto">
      <table class="w-full text-left align-middle">
        <thead><tr class="text-slate-600 text-sm"><th class="py-2">User</th><th class="py-2">Password</th><th class="py-2">Expires</th><th class="py-2">Status</th><th class="py-2"></th></tr></thead>
        <tbody class="text-sm">
          {% for r in rows %}
          <tr class="border-t">
            ...
            <td class="py-2">
              <div class="flex items-center gap-2 flex-wrap">
                ...
                <span class="text-xs px-2 py-1 rounded-full {{ 'bg-emerald-100 text-emerald-700' if r['days']>=0 else 'bg-rose-100 text-rose-700' }}">သက်တမ်း {{r['days']}} days</span>
              </div>
            </td>
            ...
            <td class="py-2">
              <form method=post action="/del/{{r['id']}}"><button class="bg-rose-600 hover:bg-rose-500 text-white px-3 py-1 rounded">🗑️</button></form>
            </td>
          </tr>
          {% endfor %}
        </tbody>
      ...
      {% if total_users > 1 %}
      <p class="text-xs text-slate-500 mt-2">Note: zivpn server log မထွက်တဲ့ VPS များတွင် per-password online mapping မရနိုင်ပါ။ single active user နဲ့ UDP activity ရှိလျှင် auto-online ပြပါတယ်။</p>
      {% endif %}
    </div>
  </section>
</main>
    ...
    default_exp=default_exp, server_ip=server_ip)
