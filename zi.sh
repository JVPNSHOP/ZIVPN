cat > zi.sh <<'BASH'
#!/bin/bash
# ZIVPN UDP + Tailwind Web Panel (fix: robust Copy buttons)
# Script By: JueHtet
...
<script>
// Robust copy: HTTPS or not (fallback with hidden textarea)
...
<main class="max-w-6xl mx-auto px-4 py-4 space-y-4">
  ...
  <!-- VPS IP + Server Time (info block ABOVE stats) -->
...
{{ msg }}
        ...
        <button class="btn-slim bg-emerald-600 text-white rounded text-[11px]" onclick="copyText({{ msg|tojson }}, this)">Copy Details</button>
      </div>
