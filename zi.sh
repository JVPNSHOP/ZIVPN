...
with db() as con:
    ...
    # Add last_multi_login column if not exists
    try:
...
def detect_multi_login():
    """Detect passwords with multiple concurrent logins without affecting active connections"""
    log_content = logs()
    if not log_content:
        return set()
    
    # Extract passwords from connection logs
    password_pattern = r'password[=\s:]+([a-zA-Z0-9]+)'
    found_passwords = re.findall(password_pattern, log_content)
    
    # Count occurrences in the last 2 minutes
    from collections import Counter
    password_counts = Counter(found_passwords)
    
    # Return passwords with multiple recent connections
    multi_login_passwords = {pwd for pwd, count in password_counts.items() if count > 1}
    return multi_login_passwords
...
def sync():
    # Just update config without restarting service to avoid disconnections
    with db() as con:
    ...
    # Only reload configuration without full restart
    try:
...
function expireMultiLogin(password){
  if(confirm('Expire this password due to multi-login?\\nPassword: ' + password)){
    fetch('/expire-multi-login', {
      method: 'POST',
...
.multi-login-badge { background: #fef3c7; color: #92400e; border: 1px solid #f59e0b; }
</style>
</head>
...
{{ msg }}
  ...
  <section class="grid grid-cols-1 sm:grid-cols-4 gap-3">
    ...
    {% endif %}
    {% if total_multi_login > 0 %}
    <div class="bg-white rounded-2xl shadow p-4 ring-1 ring-amber-200">
      <div class="text-amber-600 text-xs">Multi-Login</div>
      <div class="mt-1 text-2xl font-bold text-amber-600">{{total_multi_login}}</div>
    </div>
    {% endif %}
  </section>

  <!-- Multi-Login Alert -->
  {% if multi_login_passwords %}
  <section class="bg-amber-50 ring-1 ring-amber-200 text-amber-800 rounded-2xl p-4">
    <div class="flex items-center gap-2 mb-2">
      <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" class="text-amber-600">
        <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z"/>
      </svg>
      <h3 class="font-semibold">Multi-Login Detected</h3>
    </div>
    <p class="text-sm">The following passwords have multiple concurrent connections:</p>
    <div class="mt-2 flex flex-wrap gap-2">
      {% for pwd in multi_login_passwords %}
      <span class="multi-login-badge badge">{{ pwd }}</span>
      {% endfor %}
    </div>
    <p class="text-xs mt-2 text-amber-700">Click "Expire" button next to any password to block multi-login</p>
  </section>
  {% endif %}
  ...
  <section class="grid md:grid-cols-[320px_1fr] gap-3">
    ...
    <div class="bg-white p-3 rounded-2xl shadow ring-1 ring-slate-200 overflow-x-auto">
      <table class="w-full text-left align-middle table-tight">
        ...
        <tbody class="tiny">
          {% for r in rows %}
          <tr class="border-t {% if r.multi_login %}bg-amber-50{% endif %}">
            <td class="py-1"><span class="font-medium truncate-soft" title="{{r['username']}}">{{r['username']}}</span></td>
            <td class="py-1">
              <div class="flex items-center gap-1.5 flex-wrap">
                <code class="code-chip px-1.5 py-0.5 bg-slate-100 rounded truncate-soft max-w-[120px]" title="{{r['password']}}">{{r['password']}}</code>
                <button onclick="copyText('{{r['password']}}',this)" class="btn-slim bg-slate-800 text-white rounded text-[11px]">Copy</button>
                {% if r['multi_login'] %}
                  <span class="badge multi-login-badge">Multi-Login</span>
                {% endif %}
            ...
            <td class="py-1">
              <div class="flex items-center gap-1.5">
                ...
                {% if r['multi_login'] %}
                <button onclick="expireMultiLogin('{{r['password']}}')" class="btn-slim bg-red-600 hover:bg-red-500 text-white rounded text-[11px]">Expire</button>
                {% endif %}
...
def detect_multi_login():
    """Detect passwords with multiple concurrent logins"""
    try:
        log_content = subprocess.check_output(["journalctl","-u",SVC,"--since","-2min","-o","cat"]).decode().lower()
    except Exception:
        return set()
    ...
    # Return passwords with multiple occurrences (multi-login)
    multi_login_passwords = {pwd for pwd, count in password_counts.items() if count > 1}
    return multi_login_passwords

def actives():
    # Just get active passwords without auto-expiring
...
# Use reload instead of restart to avoid disconnecting users
try:
    subprocess.Popen(["systemctl","reload",SVC], stdout=DEVNULL, stderr=DEVNULL)
except:
    # If reload fails, try restart but this should be rare
...
echo "• Multi-login detection without server reset"
echo "• Manual expire option for multi-login passwords"
echo "• Copy and Edit buttons fixed"
echo "• Server uses reload instead of restart to maintain connections"
BASH
