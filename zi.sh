...
.code-chip{font-family:ui-monospace,Menlo,monospace; display:inline-block; max-width:220px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap}
...
{{ msg }}
  ...
  <section class="grid md:grid-cols-[320px_1fr] gap-3">
    ...
    <div class="bg-white p-3 rounded-2xl shadow ring-1 ring-slate-200 overflow-x-auto">
      <table class="w-full text-left align-middle table-tight" style="table-layout:fixed;">
        <colgroup>
          <col style="width:25%">
          <col style="width:35%">
          <col style="width:20%">
        ...
        <tbody class="tiny">
          {% for r in rows %}
          <tr class="border-t">
            <td class="py-1 align-middle"><span class="font-medium" title="{{r['username']}}">{{r['username']}}</span></td>
            <td class="py-1 align-middle">
              <div class="flex items-center gap-2">
                <code class="code-chip px-2 py-1 bg-slate-100 rounded" title="{{r['password']}}">{{r['password']}}</code>
                <button onclick="copyText('{{r['password']}}',this)" class="copy-btn px-2 py-1 bg-slate-800 text-white rounded text-[12px]">Copy</button>
