...
function copyText(t,btn){function ok(){if(btn){btn.innerHTML='✓';btn.disabled=true;setTimeout(()=>{btn.innerHTML=BTN_ICN;btn.disabled=false;},800)}};if(navigator.clipboard&&window.isSecureContext){navigator.clipboard.writeText(t).then(ok);}else{const ta=document.createElement('textarea');ta.value=t;document.body.appendChild(ta);ta.select();document.execCommand('copy');document.body.removeChild(ta);ok();}}
...
const BTN_ICN='<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M16 1H4c-1.1 0-2 .9-2 2v12h2V3h12V1zm3 4H8c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h11c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm0 16H8V7h11v14z"/></svg>';
</script>
<style>
.table-tight td,.table-tight th{padding-top:.15rem;padding-bottom:.15rem}
.tiny{font-size:12px;line-height:1.1}
.code-chip{font-family:ui-monospace,Menlo,monospace;word-break:break-all;white-space:normal}
...
col.user{width:26%} col.pass{width:34%} col.date{width:18%} col.status{width:12%} col.act{width:10%}
/* buttons */
.icon-btn{width:28px;height:28px;display:grid;place-items:center;border-radius:.5rem;background:#0f172a;color:#fff}
.icon-btn:hover{background:#1f2937}
/* status */
.status-cell{overflow:visible}
.status-chip{display:inline-flex;align-items:center;gap:.35rem;padding:.2rem .5rem;border-radius:9999px;font-size:12px;line-height:1;white-space:nowrap}
.status-ok{background:#d1fae5;color:#065f46}
.status-off{background:#e5e7eb;color:#374151}

/* ===== Mobile: show EVERYTHING neatly (≤520px) ===== */
.act-btn{display:inline-flex;align-items:center;gap:.25rem;border-radius:.5rem;padding:.125rem .5rem;font-size:11px}
.act-btn.edit{background:#f59e0b;color:#fff}
.act-btn.del{background:#dc2626;color:#fff}
...
@media (max-width:520px){
  ...
  .tiny{font-size:11px}
  th{font-size:12px}
  .code-chip{font-size:11px;padding:.15rem .4rem}
  .badge{font-size:10px;padding:.15rem .4rem}
  .icon-btn{width:26px;height:26px}

  /* tighter column plan so all columns fit */
  col.user{width:28%}
  col.pass{width:30%}
  col.date{width:22%}
  col.status{width:20%}
  col.act{width:56px} /* fixed actions width */

  /* status never clipped */
  .status-chip{min-width:70px}

  /* actions = icon-only on mobile */
  .act-btn{width:26px;height:26px;justify-content:center;padding:0}
  ...
  <section class="grid md:grid-cols-[320px_1fr] gap-3">
    <div class="bg-white p-3 rounded-2xl shadow ring-1 ring-slate-200">
      <h3 class="font-semibold mb-2 text-sm flex items-center gap-2">
        <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" class="text-emerald-500"><path d="M12 12c2.761 0 5-2.686 5-6s-2.239-6-5-6-5 2.686-5 6 5 6zm0 2c-4.418 0-8 2.239-8 5v3h16v-3c0-2.761-3.582-5-8-5z"/></svg>
        Add / Update User
      </h3>
    ...
    <div class="bg-white p-3 rounded-2xl shadow ring-1 ring-slate-200 overflow-x-auto">
      <table class="w-full text-left align-middle table-tight table-fixed">
        <colgroup><col class="user"><col class="pass"><col class="date"><col class="status"><col class="act"></colgroup>
        <thead>
        ...
        <tbody class="tiny">
          ...
          <tr class="border-t">
            ...
            <td class="py-1">
              <div class="flex items-center gap-2">
                <code class="code-chip px-2 py-1 bg-slate-100 rounded" title="{{r['password']}}">{{r['password']}}</code>
                <button onclick="copyText('{{r['password']}}',this)" class="icon-btn" aria-label="Copy" title="Copy">{{''}}</button>
                <script>document.currentScript.previousElementSibling.innerHTML=BTN_ICN</script>
            ...
            <td class="py-1 status-cell">
              {% if not r['expired'] %}
                <span class="status-chip status-ok"><span class="w-2 h-2 rounded-full bg-emerald-500 inline-block"></span>Online</span>
              {% else %}
                <span class="status-chip status-off"><span class="w-2 h-2 rounded-full bg-slate-400 inline-block"></span>Offline</span>
              {% endif %}
            </td>
            <td class="py-1">
              <div class="flex items-center gap-1.5 justify-end">
                <button type="button" onclick="fillForm('{{r['username']}}','{{r['password']}}','{{r['expires']}}')" class="act-btn edit" title="Edit" aria-label="Edit">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M3 17.25V21h3.75l11-11-3.75-3.75-11 11zM20.71 7.04a1 1 0 0 0 0-1.41l-2.34-2.34a1 1 0 0 0-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z"/></svg>
                  <span class="btn-label">Edit</span>
                </button>
                <form method=post action="/del/{{r['id']}}" onsubmit="return confirm('Delete {{r['username']}} ?')">
                  <button class="act-btn del" title="Delete" aria-label="Delete">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M9 3h6l1 1h4v2H4V4h4l1-1zm1 7h2v8h-2v-8zm4 0h2v8h-2v-8zM7 10h2v8H7v-8z"/></svg>
...
# --- OPTIONAL: Server-side keepalive ---
