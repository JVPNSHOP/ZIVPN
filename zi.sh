cat > zi.sh <<'BASH'
#!/bin/bash
# ZIVPN UDP Server + Tailwind Web Panel (auto-refresh + robust copy)
# Script By: JueHtet
# Update:
#  - Copy button now works across browsers (HTTPS/HTTP) via fallback
#  - ONLINE = not expired, OFFLINE = expired (simple status by validity)
...
DEBIAN_FRONTEND=noninteractive apt-get install -yq python3-venv python3-pip openssl ufw curl jq
...
def active_rows():
    ...
    with db() as con:
        for r in con.execute("SELECT * FROM users"):
            ...
            expired = exp < today
            # Requested logic: online if NOT expired, offline if expired
            online = (not expired)
            rows.append({
...
def index():
    ...
    total_offline=sum(1 for r in rows if (not r["online"]))
  ...
  <script>
    ...
    function copyToClipboard(text, btn){
      if (navigator.clipboard && navigator.clipboard.writeText){
        navigator.clipboard.writeText(text).then(()=>{btn.innerText='✓ Copied'; setTimeout(()=>btn.innerText='Copy',800);})
        .catch(()=>{copyTextFallback(text); btn.innerText='✓ Copied'; setTimeout(()=>btn.innerText='Copy',800);});
      } else {
        copyTextFallback(text); btn.innerText='✓ Copied'; setTimeout(()=>btn.innerText='Copy',800);
      }
    }
  </script>
</head>
          ...
          <tr class="border-t">
            ...
            <td class="py-2">
              <div class="flex items-center gap-2 flex-wrap">
                ...
                <button type="button" onclick="copyToClipboard('{{r['password']}}',this)" class="bg-slate-800 text-white px-2 py-1 rounded">Copy</button>
                <span class="text-xs px-2 py-1 rounded-full {{ 'bg-emerald-100 text-emerald-700' if r['days']>=0 else 'bg-rose-100 text-rose-700' }}">{{r['days']}} days</span>
              </div>
            </td>
