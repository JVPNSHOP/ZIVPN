cat > zi.sh <<'BASH'
#!/bin/bash
# ZIVPN UDP + Tailwind Web Panel (No-Drop Save + Multi-Login Allowed)
# Base Script By: JueHtet
# Mod: No-drop sync, multi-login allowed, manual Apply Now
set -euo pipefail
...
# ---- systemd unit (ExecReload via HUP; restart never used by panel save) ----
...
# Prefer graceful reload (SIGHUP) if supported by the binary.
...
# ---------------- Flask app (No-Drop Save + Multi-Login Allowed) ----------------
cat > "${APP_PY}" <<'PY'
#!/usr/bin/env python3
...
# Multi-login is ALLOWED; return empty list so UI never flags
    ...
    with db() as con:
        for r in con.execute("SELECT * FROM users ORDER BY username"):
            ...
            rows.append({
                ...
                "days_left": days_left(r["expires"]), "is_multi_login": False
            })
    return rows

def write_cfg_only():
    """Write-only sync: update config file, NEVER touch service here (No-Drop Save)."""
    with db() as con:
...
{{ msg }}
      ...
      <div class="mt-1 text-[11px] text-emerald-800/80">Shared passwords are allowed</div>
    </div>
    {% endfor %}
  {% endif %}
  {% endwith %}
  ...
  <section class="grid md:grid-cols-[320px_1fr] gap-3">
    <div class="bg-white p-3 rounded-2xl shadow ring-1 ring-slate-200">
      ...
      <form method=post action="/save" class="space-y-2">
        ...
        <button class="w-full bg-emerald-600 hover:bg-emerald-500 text-white py-2 rounded-xl shadow text-sm">💾 Save (No-Drop)</button>
      </form>
      <form method="post" action="/apply" class="mt-2">
        <button class="w-full bg-slate-800 hover:bg-slate-700 text-white py-2 rounded-xl shadow text-sm">↻ Apply Now (Try Reload)</button>
        <p class="mt-1 text-[11px] text-slate-500">Save only doesn’t disconnect users. Apply tries SIGHUP reload; if unsupported, it’s skipped.</p>
      </form>
      <p class="mt-2 text-[11px] text-slate-500">Script By: <b>JueHtet</b> • Mod: No-Drop & Multi-Login</p>
    </div>
...
def save():
    ...
    msg=f"IP : {ip}\nUsers : {u}\nPassword : {p}\nExpired Date : {e}\nShared passwords allowed"
    flash(msg, "ok")
    # Write-only sync (no reload/restart) to avoid disconnects
    try:
        write_cfg_only()
    except Exception:
        pass
...
# -------- Manual Apply (Try Reload) --------
@app.route("/apply", methods=["POST"])
@login_required
def apply_now():
    """Best-effort NO-DROP apply: try SIGHUP; if unsupported, silently skip."""
    try:
        ...
        flash("Config applied (reload attempted).", "ok")
    except Exception:
        flash("Applied in write-only mode (no reload).", "ok")
    return redirect("/")
...
# ---- Sync service & timer (write-only; harmless) ----
...
echo
echo "✅ INSTALL COMPLETE (No-Drop Save + Multi-Login Allowed)"
echo "Admin Panel: http://$(hostname -I | awk '{print $1}'):8088/login"
echo "Admin User: ${ADMIN_USER}"
echo "Admin Pass: ${ADMIN_PASSWORD}"
echo "Tips:"
echo " - 💾 Save: updates config on disk only (connections stay up)."
echo " - ↻ Apply Now: tries SIGHUP reload (no-drop). If unsupported, it's skipped."
echo "======================================"
BASH
