...
def active_rows():
    ...
    # Check for duplicate passwords and expire them
    with db() as con:
        ...
        # Expire all but the first occurrence of duplicate passwords
        for dup in duplicates:
            password = dup["password"]
            first_user = con.execute("""
                SELECT id FROM users 
                WHERE password = ? AND DATE(expires) >= DATE('now') 
                ORDER BY created_at ASC 
                LIMIT 1
            """, (password,)).fetchone()
            
            if first_user:
                # Expire all other users with same password
                con.execute("""
                    UPDATE users 
                    SET expires = DATE('now', '-1 day') 
                    WHERE password = ? AND id != ? AND DATE(expires) >= DATE('now')
                """, (password, first_user["id"]))
    
    with db() as con:
        for r in con.execute("SELECT * FROM users ORDER BY created_at DESC"):
...
def save():
    ...
    # Check if password already exists and expire the old one
    with db() as con:
        existing = con.execute("SELECT id, username FROM users WHERE password = ? AND DATE(expires) >= DATE('now')", (p,)).fetchall()
        if existing:
            # Expire all existing users with same password
            for user in existing:
                con.execute("UPDATE users SET expires = DATE('now', '-1 day') WHERE id = ?", (user["id"],))
