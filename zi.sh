...
# - Script2 UI on Script1 logic
# - Multi-Login enabled (no DISTINCT)
# - No-drop tweaks: reload first; if not supported, debounce restart
# Script By: JueHtet (merged & improved)
...
# ---- Flask app (Script2 HTML + improved sync) ----
cat > "${APP_PY}" <<'PY'
#!/usr/bin/env python3
...
DB=os.path.join(os.getenv("STATE_DIR","/var/lib/zivpn-admin"),"zivpn.db")
...
LAST_HASH=os.path.join(STATE_DIR,"auth.sha256")
DEBOUNCE=os.path.join(STATE_DIR,"restart_at")  # epoch seconds
...
def write_cfg(pw_list):
    cfg={}
    try: cfg=json.load(open(ZIVPN_CFG))
    except Exception: pass
    cfg.setdefault("auth",{})["mode"]="passwords"
    cfg["auth"]["config"]=pw_list
    cfg["config"]=pw_list
    with tempfile.NamedTemporaryFile("w",delete=False) as f:
        json.dump(cfg,f,indent=2); tmp=f.name
    os.replace(tmp,ZIVPN_CFG)

def need_restart(new_pw):
    """hash current active-password list; restart only if changed"""
    h=hashlib.sha256("\n".join(new_pw).encode()).hexdigest()
    if os.path.exists(LAST_HASH):
        if open(LAST_HASH).read().strip()==h:
            return False
    with open(LAST_HASH,"w") as f: f.write(h)
    return True

def schedule_bounce(delay=30):
    """debounce restarts to avoid rapid bouncing / reduce drops"""
    t=int(time.time())+delay
    with open(DEBOUNCE,"w") as f: f.write(str(t))

    # background scheduler: if another write happens within delay, the later one wins
    def _bg():
        time.sleep(delay+2)
        try:
            with open(DEBOUNCE) as f: target=int(f.read().strip())
        except Exception:
            return
        if target>int(time.time()):  # another write extended window
            return
        # try non-disruptive reload first; fallback to restart
        subprocess.Popen(["systemctl","reload",ZIVPN_SVC], stdout=DEVNULL, stderr=DEVNULL)
        # small wait then check if reload changed mainpid; if not, do restart
        time.sleep(2)
        # fallback (best-effort, still background)
        subprocess.Popen(["systemctl","restart",ZIVPN_SVC], stdout=DEVNULL, stderr=DEVNULL)
    subprocess.Popen(["/usr/bin/python3","-c",f"import time,subprocess,sys; time.sleep({delay+2}); subprocess.run(['true'])"], stdout=DEVNULL, stderr=DEVNULL)  # placeholder to keep process slot

    # use a lightweight python thread via systemd is tricky; simple nohup shell:
    subprocess.Popen(["bash","-lc",f"(sleep {delay+2}; "
                      f"if [ -f '{DEBOUNCE}' ] && [ $(cat '{DEBOUNCE}') -le $(date +%s) ]; then "
                      f"systemctl reload {ZIVPN_SVC} >/dev/null 2>&1 || true; "
                      f"sleep 2; systemctl restart {ZIVPN_SVC} >/dev/null 2>&1 || true; "
                      f"fi ) &"], stdout=DEVNULL, stderr=DEVNULL)

def sync():
    # Multi-Login enabled: do NOT DISTINCT
    with db() as con:
        pw=[r[0] for r in con.execute("SELECT password FROM users WHERE DATE(expires)>=DATE('now')")]
    if not pw: pw=["zi"]

    write_cfg(pw)

    # only bounce if auth list really changed
    if need_restart(pw):
        # debounce restart to reduce drops; reload first if supported
        schedule_bounce(delay=30)
...
function copyText(t, btn){
  function ok(){ if(btn){ btn.innerText='✓'; btn.disabled=true; setTimeout(()=>{btn.innerText=''; btn.disabled=false;},800);} }
...
<div class="fab">
  <a href="https://t.me/Pussy1990" target="_blank" rel="noopener" class="bg-sky-600 hover:bg-sky-500 text-white" title="Telegram">
    <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M9.04 15.47l-.39 5.49c.56 0 .8-.24 1.09-.53l2.62-2.52 5.43 3.97c1 .55 1.71.26 1.98-.93ল3.6-16.85c.32-1.5-.54-2.09-1.52-1.73L1.16 9.64c-1.46.57-1.44 1.39-.25 1.76l5.34 1.66L19.36 6.1c.62-.41 1.18-.18.72.23"/></svg>
  </a>
...
# --- Auto Sync Script (same behavior) ---
cat > "${SYNC_PY}" <<'PY'
...
DEBOUNCE=os.path.join(STATE,"restart_at")
...
def need_restart(pw):
    h=hashlib.sha256("\n".join(pw).encode()).hexdigest()
    if os.path.exists(LAST_HASH) and open(LAST_HASH).read().strip()==h:
        return False
    open(LAST_HASH,"w").write(h); return True

pw=actives()
write_cfg(pw)
if need_restart(pw):
    # debounce 30s; reload first then restart (background)
    t=int(time.time())+30
    open(DEBOUNCE,"w").write(str(t))
    subprocess.Popen(["bash","-lc",
        "(sleep 32; if [ -f '%s' ] && [ $(cat '%s') -le $(date +%%s) ]; then "
        "systemctl reload %s >/dev/null 2>&1 || true; "
        "sleep 2; systemctl restart %s >/dev/null 2>&1 || true; fi) &" % (DEBOUNCE,DEBOUNCE,SVC,SVC)
