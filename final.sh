...
# Script By: JueHtet — Online by password, show run IP, label "သက်တမ်း", default date = today
...
def active_rows():
    ...
    with db() as con:
        for r in con.execute("SELECT * FROM users"):
            ...
            expired = exp < today
            online = (not expired) and (r["password"].lower() in log)  # password-based online
            rows.append({
